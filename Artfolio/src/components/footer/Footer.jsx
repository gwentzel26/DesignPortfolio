    ...
    <footer className='bg-gray-900 font-arima text-white py-10 px-6 mt-20 relative'>
        <div className='max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8'>
            <div data-aos='fade-up' data-aos-delay='400' className='bg-orange-500 p-4 flex flex-col justify-center -mt-20'>
                ...
                <form className='flex flex-col space-y-2'>
                    <input type="email" placeholder='Eamil' className='w-full p-2 text-gray-900' />
      ...
      <ul>
        {Links.map(({id, child, href, style, download}) => (
          <li key={id} className={
            "flex justify-between items-center w-40 h-12 px-4 bg-orange-400 ml-[-100px] hover:ml-[-10px] hover:rounded-md duration-300" 
            + " " + style}>
            <a
              href={href}
